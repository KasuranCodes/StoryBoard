...
// #define MODEL "assets/models/test/thingy.obj"

// #define MODEL "assets/models/blue42/blue42.obj"
#define MODEL "assets/models/tri/tri.obj"
...
int init(void) {
    ...
    obj.position = (vec3) {0, 0, 15};

    // triNum = 1;
    
    triNum = obj.triNum;
    printf("Total Tris Loaded: %d\n", triNum);
    ...
    // Attach Camera to context
    context.menuState = 0;
    context.cam = cameraCreate(cameraPosition, cameraDirection, fFov, windowWidth, windowHeight, fAspect, fNear, fFar);
...
int frame(void) {
    ...
    //     drawTri(triQueue[i].v, triQueue[i].color);
    // }

    // for (int i = 0; i < triNum; i++) {
    //     rotateTri('y', elapsedTime, &triQueue[i], &rotatedTriangles[i]);
    // }
    // renderPerspective(rotatedTriangles, triNum, &tempTriNum, perspectiveTriangles, &context.cam);
    // // printf("tempTriNum\t|\t\t%d\n", tempTriNum);
