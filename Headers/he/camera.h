...
static void rotateObj(char axis, float fTheta, Object *in, Object *out);

static void renderPerspective(Object *in, int triNum, int *newNum, tri2d *out, Camera *cam);
...
static void rotateObj(char axis, float fTheta, Object *in, Object *out) {
    for (int i = 0; i < in->triNum; i++) {
        rotateTri(axis, fTheta, &in->tris[i], &out->tris[i]);
    }
}
...
static void renderObj(Object *in, int triNum, int *newNum, tri2d *out, Camera *cam) {
    if (in == NULL || out == NULL || cam == NULL) {
        fprintf(stderr, "Error: Null pointer argument in renderObj.\n");
        return;
    }
